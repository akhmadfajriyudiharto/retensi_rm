    ...
    public function pemusnahan()
    {
        $fields = [
            ...
            'status' => [
                'name'      => 'Status',
                'type'      => 'select',
                'options'   => Arr::only(RetensiRecord::getAllStatus(), ['BM','M']),
                'isCreate'  => false,
                'isEdit'    => false
            ],
        ];
        $pageSetting = [
            'title'         => 'List Pemusnahan',
            'model'         => \App\Models\RekamMedis::class,
            'routeName'     => 'admin.pemusnahan',
            'formWidth'     => 'fullscreen',
            'fieldWidth'    => 6,
            'fields'         => $fields
        ];
        if (request()->ajax()) {
            ...
            $data = RekamMedis::select(['rekam_medis.id', 'rekam_medis.tanggal_kunjungan', 'rekam_medis.diagnosa',
                            ...
                            ->whereIn('retensi_records.status', [RetensiRecord::STATUS_INAKTIF, RetensiRecord::STATUS_MUSNAH])
                            ->whereDate('rekam_medis.batas_inaktif', '<', now());
            return DataTables::of($data)
                ->filterColumn('no_rm', function($query, $keyword) {
                    $query->where('pasiens.no_rm', 'ilike', "%{$keyword}%");
